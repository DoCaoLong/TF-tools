...
class Timefarm {
    ...
    async login(data) {
        ...
        try {
            ...
            if (res.status !== 200) {
                this.log(
                    colors.red(
                        `Login không thành công! Mã trạng thái: ${res.status}`
                    )
                );
                return null;
            }
            ...
            this.log(
                colors.red(`Lỗi trong quá trình đăng nhập: ${error.message}`)
            );
            return null;
        }
    }
    ...
    async upgradeWatch() {
        ...
        try {
            ...
            this.log(
                colors.green(
                    `Nâng cấp thành công đồng hồ lên lv ${level}, balance ${balance}`
                )
            );
        } catch (error) {
    ...
    async getBalance(upgradeWatch) {
        ...
        while (true) {
            try {
                ...
                const timestamp =
                    DateTime.fromISO(data.activeFarmingStartedAt).toMillis() /
                    1000;
                ...
                this.log(
                    colors.red("Lỗi kết nối hoặc truy vấn không thành công")
                );
    ...
    async countdown(t) {
        for (let i = t; i > 0; i--) {
            ...
            const minutes = String(Math.floor((i % 3600) / 60)).padStart(
                2,
                "0"
            );
            const seconds = String(i % 60)
                .padStart(2, "0")
                .split(".")[0];
            process.stdout.write(
                colors.white(
                    `[*] Cần chờ ${hours}:${minutes}:${seconds}     \r`
                )
            );
    ...
    async main() {
        // Thiết lập mặc định cho việc nâng cấp đồng hồ
        const upgradeWatch = true; // Thay vì phải hỏi người dùng, luôn nâng cấp đồng hồ
        ...
        const datas = fs
            ...
            .filter(
                (line) =>
                    line.length > 0 &&
                    decodeURIComponent(line).includes("user=")
            );
        ...
        while (true) {
            ...
            for (let i = 0; i < datas.length; i++) {
                ...
                if (!token) {
                    this.log(
                        colors.red(
                            "Không thể đọc token, đang gửi yêu cầu đăng nhập!"
                        )
                    );
